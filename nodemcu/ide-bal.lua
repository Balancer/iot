...
local function editor(aceEnabled) -- feel free to disable the shiny Ajax.org Cloud Editor
 ...
 srv:listen(80, function(conn)
  ...
  conn:on("receive", function(sck, payload)
    ...
    sen = sen .. "<html><head><link rel='stylesheet' href='https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css' />"
        .. '<script src="https://cdnjs.cloudflare.com/ajax/libs/tablesort/5.0.2/tablesort.min.js"></script>'
        .. '<script src="https://cdnjs.cloudflare.com/ajax/libs/tablesort/5.0.2/sorts/tablesort.number.js"></script>'
        .. '<style>.ace_editor {font-size: 16pt!important}</style>'
        .. "</head><body><div class='container'><h1><a href='/'>NodeMCU IDE</a></h1>"

    if vars ~= nil then
        local args = parseurl(vars)

        if args.cmd ~= nil then
            if file.exists("ide-cmd-" .. args.cmd .. ".lua") then
                sck:send(loadfile("ide-cmd-" .. args.cmd .. ".lua")(args))
                return
            end
        end
    end
    ...
    elseif vars == "run" then
        ...
        tmr.alarm(0, 1000, tmr.ALARM_SINGLE, function() 
            ...
            sen = sen .. "</verbatim></div></body></html>"
            sck:send(sen)
        end)

        return

    elseif vars == "list" then
        sen = ""
        for file, v in pairs(file.list()) do
            sen = sen .. file .. "\n"
        end
    ...
    if url == "" then
        local l = file.list();
        message[#message + 1] = "<table id='tab' class='table table-bordered table-striped table-hover table-condensed'><thead><tr><th data-sort-default>Name</th><th>Size</th><th>Edit</th><th>Run</th><th>Compile</th><th>Delete</th></tr></thead><tbody>"
        ...
        message[#message + 1] = "</tbody></table><a class='btn btn-warning' href='#' onclick='v=prompt(\"Filename\");if (v!=null) { this.href=\"/\"+v+\"?edit\"; return true;} else return false;'>Create new</a> &nbsp; &nbsp; "
    message[#message + 1] = "<a class='btn btn-danger' href='#' onclick='var x=new XMLHttpRequest();x.open(\"GET\",\"/?restart\");x.send();setTimeout(function(){location.href=\"/\"},5000);this.innerText=\"Please wait\";return false'>Restart</a>"
    end
    message[#message + 1] = "</div>"
        .. "<script> new Tablesort(document.getElementById('tab'));  </script>"
        .."<br/><br/></body></html>"
